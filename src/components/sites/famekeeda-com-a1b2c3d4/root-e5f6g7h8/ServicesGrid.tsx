...
const services = [
  ...
  },
  {
    ...
    variant: "dark" // Dark grey background, white text
  },
  {
          ...
          {services.map((service, idx) => {
            ...
            return (
              <a 
                key={idx}
                href={service.href}
                className={`relative overflow-hidden rounded-[2rem] p-8 md:p-12 min-h-[360px] md:min-h-[420px] flex flex-col justify-between group transition-transform hover:-translate-y-1 shadow-sm hover:shadow-xl ${
                  isLight ? "bg-white text-fk-dark" : "bg-fk-charcoal text-white"
                }`}
              >
                <div className="relative z-10">
                  <h3 className="text-3xl md:text-4xl font-bold mb-6 max-w-[200px] leading-tight">
                  ...
                  <div className={`inline-flex items-center gap-2 font-semibold text-sm border-b pb-1 transition-colors ${
                    isLight ? "border-fk-dark hover:text-fk-red hover:border-fk-red" : "border-white hover:text-gray-300 hover:border-gray-300"
                  }`}>
                    Know more
                ...
                {/* Floating Image */}
                <div className="absolute right-0 bottom-0 w-2/3 h-2/3 md:w-3/5 md:h-3/4 transition-transform duration-500 group-hover:scale-105 origin-bottom-right">
                  <Image 
                    src={service.img} 
                    alt={service.title}
                    fill
